...
test_that("delete_experiment", {
  ...
  expect(delete_experiment(get_experiment_id_from_name(get_project_id_from_name("PROJECT_TESTU"), "EXPERIMENT_EXTERNAL_REGRESSION_TESTU")) == 204, "delete_experiment() doesn't retrieve a 204 status code for EXPERIMENT_EXTERNAL_REGRESSION_TESTU")
  expect(delete_experiment(get_experiment_id_from_name(get_project_id_from_name("PROJECT_TESTU"), "EXPERIMENT_EXTERNAL_CLASSIFICATION_TESTU")) == 204, "delete_experiment() doesn't retrieve a 204 status code for EXPERIMENT_EXTERNAL_CLASSIFICATION_TESTU")
  expect(delete_experiment(get_experiment_id_from_name(get_project_id_from_name("PROJECT_TESTU"), "EXPERIMENT_EXTERNAL_MULTICLASSIFICATION_TESTU")) == 204, "delete_experiment() doesn't retrieve a 204 status code for EXPERIMENT_EXTERNAL_MULTICLASSIFICATION_TESTU")
})
