...
get_usecase_version_predictions <- function(usecase_version_id, generating_type = "user") {
  ...
  while(T) {
    if(generating_type == "user") {
      resp <- pio_request(paste0('/usecase-versions/', usecase_version_id, '/predictions?page=', page), GET)
    }
    else {
      resp <- pio_request(paste0('/usecase-versions/', usecase_version_id, '/holdout-predictionspredictions?page=', page), GET)
    }
    resp_parsed <- content(resp, 'parsed', encoding = "UTF-8")
...
get_prediction_infos <- function(prediction_id) {
  ...
  resp <- pio_request(paste0('/predictions/', prediction_id), GET)
  resp_parsed <- content(resp, 'parsed', encoding = "UTF-8")
...
get_prediction <- function(prediction_id, time_out = 3600, wait_time = 10) {
  ...
  while(attempt < time_out/wait_time) {
    temp <- tempfile()
    resp <- pio_download(paste0('/predictions/', prediction_id, '/download'), temp)
